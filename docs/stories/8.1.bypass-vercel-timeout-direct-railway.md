...
- [ ] **Task 1: Investigate Railway Backend API** (AC: 1, 2, 8)
  - [ ] Find Railway backend URL (check env vars `NEXT_PUBLIC_BACKEND_URL` in Vercel dashboard)
  - [ ] Identify the Railway endpoint for document analysis (review backend code)
  - [ ] Determine authentication mechanism (Clerk JWT validation? API key? None?)
  - [ ] Document the expected request/response format
  - [ ] Test Railway endpoint manually with curl or Postman

- [ ] **Task 2: Update Frontend Analysis Call** (AC: 1, 2, 10, 11)
  - [ ] Locate the frontend code that calls `/api/analyze-document` (likely in `/app/analyze/[uploadId]/page.tsx`)
  - [ ] Get Clerk authentication token using `useAuth()` hook
  - [ ] Replace API call with direct Railway backend URL
  - [ ] Include Clerk token in `Authorization: Bearer <token>` header
  - [ ] Update error handling for Railway API responses
  - [ ] Add loading states and error messages for network failures
  - [ ] Test with sample document upload

- [ ] **Task 3: Verify CSP Configuration** (AC: 9)
  - [ ] Check `middleware.ts` CSP `connect-src` directive
  - [ ] Confirm `https://*.railway.app` is already allowed (should be from Story 7.8)
  - [ ] Test browser console for CSP violations after implementation
  - [ ] Add Railway domain if missing

- [ ] **Task 4: Verify Polling & Webhook Still Work** (AC: 3, 4, 12)
  - [ ] Confirm `/api/runs` polling logic still functions correctly
  - [ ] Test webhook endpoint `/api/runs/[runId]/complete` receives Railway callbacks
  - [ ] Ensure frontend updates status correctly after analysis completes
  - [ ] Verify Vercel Blob upload flow remains unchanged

- [ ] **Task 5: Remove or Deprecate Old API Route** (AC: 5)
  - [ ] Delete `/app/api/analyze-document/route.ts` (no longer needed)
  - [ ] Remove any unused imports or dependencies
  - [ ] Search codebase for any remaining references to `/api/analyze-document`
  - [ ] Update any documentation or comments

- [ ] **Task 6: End-to-End Testing** (AC: 6, 7)
  - [ ] Upload and analyze a document that takes >10 seconds
  - [ ] Verify no 504 timeout errors occur
  - [ ] Confirm analysis completes and results display correctly
  - [ ] Test with multiple document types (PDF, etc.)
  - [ ] Verify opportunity cards appear in sidebar after completion
  - [ ] Test error scenarios (network failure, authentication failure, Railway backend down)
