...
- **File**: `app/api/status/[runId]/route.ts`
  - **Change**: Added brand_name to API response by loading from metadata.json
  - **Why**: AC 14 required company name in header, but API wasn't providing brand_name field
  - **How**: Added metadata.json loading logic (lines 122-138) to populate brand_name in response contract, ensuring header badge displays correctly
...
- [ ] Consider replacing hardcoded `join(process.cwd(), '..')` with environment variable
- [ ] Add exponential backoff to retry logic (currently fixed 5s delay)
- [ ] Add unit tests for calculateStageStatus() helper function
...
**Issues identified:**
1. **Hardcoded project root assumption** (`join(process.cwd(), '..')` at line 86 in API route)
   - Fragile across different deployment environments (local dev vs Vercel)
   - Recommendation: Use environment variable for data directory path
2. **No exponential backoff** for retry logic
   - Single retry with fixed 5s delay may not handle sustained network issues
   - Recommendation: Implement exponential backoff (5s → 10s → 20s)
3. **Missing brand_name in API response** (FIXED during review)
   - Required metadata.json file creation during pipeline execution

**Monitoring recommendations:**
- Track API route failures in production logs for path resolution issues
- Monitor polling retry patterns to validate single-retry sufficiency

### Files Modified During Review

**Modified:**
- `app/api/status/[runId]/route.ts` (lines 122-151) - Added brand_name loading from metadata.json

**Developer: Please update File List in Dev Agent Record section.**

### Gate Status

**Gate: CONCERNS** → `docs/qa/gates/4.1-vertical-pipeline-viewer-ui.yml`
...
- 3 medium-severity reliability concerns identified (not blocking for MVP)
- No automated test coverage (acceptable for hackathon, risky for production)
...
- Add automated test suite before production scale-up
