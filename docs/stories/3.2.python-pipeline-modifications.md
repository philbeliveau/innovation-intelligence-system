...
3. Create `run_from_uploaded_file(input_file_path, brand_id, run_id)` function that executes all 5 stages sequentially
4. Function creates output directory `data/test-outputs/{run_id}/` and sets up logging to `{run_id}/logs/pipeline.log`
5. Function reads PDF using PyPDF/pypdf library and extracts text content
6. Function logs stage start/completion messages (e.g., "Starting Stage 1", "Stage 1 execution completed")
7. Modify `Stage1Chain.save_output()` to generate both markdown and JSON files
8. JSON file structure: `{ selected_tracks: [1, 2], track_1: { title, summary, icon_url }, track_2: { title, summary, icon_url }, completed_at }`
9. Add `_parse_inspirations(output)` method to extract track data from markdown with fallback handling
10. Update `STAGE1_TEMPLATE` prompt to explicitly request exactly 2 tracks in parseable format
11. Existing CLI modes (`--batch`, `--input`, `--brand`) continue working unchanged
12. Pipeline output quality and format remain consistent between CLI and web execution
